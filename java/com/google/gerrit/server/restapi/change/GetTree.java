...
public class GetTree implements RestReadView<RevisionResource> {
  ...
  @Override
  public Response<TreeInfo> apply(RevisionResource rsrc) throws IOException, RestApiException {
  ...
  private TreeInfo toTreeInfo(Repository repo, RevTree tree) throws IOException, RestApiException {
    ...
    try (RevWalk rw = new RevWalk(repo); TreeWalk tw = new TreeWalk(repo)) {
      ...
      if (!Strings.isNullOrEmpty(path)) {
        try {
          ...
          throw new ResourceNotFoundException("Can't find " + path + " in the repository");
        }
      }
  ...
  private ObjectId getSubTree(RevWalk rw, RevTree tree, String path) throws IOException {
